...
Client.prototype.writeStream = function(destination, callbackStream, callbackComplete) {
  var self = this;

  var alreadyCalledBack = false;
  function callbackOnce(err, resp){
    if(alreadyCalledBack) return;
    callbackComplete(err, resp);
  ...
  self.sftp(function(err, sftp){
    ...
    var sftp_writeStream = sftp.createWriteStream(destination)
    sftp_writeStream.on('error', function(err){
      callbackOnce(err);
    });
    sftp_writeStream.on('close',function(){
          self.emit('read', destination);
          callbackOnce(null, 'complete');
        })
        .on('error', function(err){
          callbackOnce(err);
        })
    ...
    callbackStream(err, sftp_writeStream);
  });

  return self;
};

Client.prototype.downloadStream = function(src, stream, callbackComplete) {
  var self = this;
...
Client.prototype.downloadStream = function(src, stream, callback) {
  var self = this;
